...
static inline void detect(double w0, double a0, unsigned long ts) {
  ...
  data->e[data->i] = data->envelope;   // store envelope alongside signal

  if (data->i >= 50) {
    double best_corr = -1e9;
    for (int shift = 0; shift < 50; shift += 8) {  // coarse search
      double corr = 0.0;
      for (int i = 0; i < 50; ++i) {
    ...
    if (best_corr < 4 && best_corr > 2.30) {   // closer to 0 than 1
      notifyHeadNod(2, ts);
    }
    data->i = 0;
  }
